...
const ListProfiles = ({ showModal, data, onChange, onSave }: PropProfile) => {

    const [formData, setformData] = useState({} as IModelProfile);

    const EditRender = (text: string, record: IModelProfile, index: number) => {
        return (
            <Button key={record.id} icon={<EditOutlined />} type="ghost" onClick={(e) => { setformData(record); onChange(e); }}></Button>
        );
    }
    ...
    return (
        <React.Fragment>
            ...
            <Modal title="Info Client." open={showModal} destroyOnClose={true} footer={false} >
